...
def main(filename_primitive, args):
	flgSkinned = False
#	pdb.set_trace()
	...
	root = wot.readXml(filename_visual)
...
#			pdb.set_trace()
			...
			if has_uv2:
				...
				if uv2_format == 'set3/uv2pc':
					for i in range(uv2_Count):
						u, v = unpack('<2f', mainFP.read(8))
						uv2_list.append( (u, 1-v) )
			"""
			...
			#set 0.9.12 new format flag
			if "BPVT" in vertices_subname:
				flgNewFormat = True
				#null dword used to be vertex length
				mainFP.read(4)
...
#					pdb.set_trace()
					...
					elif vertices_subname.endswith("xyznuvtb"):
						vert.t = unpack('I', mainFP.read(4))[0]
						vert.bn = unpack('I', mainFP.read(4))[0]
					...
					#new format skinned. Data structure is guess work 
						...
						vert.indexB_1 = unpack('B', mainFP.read(1))[0]
						vert.indexB_2 = unpack('B', mainFP.read(1))[0]
						vert.indexB_3 = unpack('B', mainFP.read(1))[0]
						# weight_1:
						vert.weight_1 = unpack('B', mainFP.read(1))[0]/255.0
						# weight_2:
						vert.weight_2 = unpack('B', mainFP.read(1))[0]/255.0
						# weight_3:
						vert.weight_3 = 1.0 - vert.weight_1 - vert.weight_2
						vert.t = unpack('I', mainFP.read(4))[0]
						vert.bn = unpack('I', mainFP.read(4))[0]
...
#							pdb.set_trace()
					...
					#old format skinned
						...
						print("TO DO: %s %s" % (vertices_subname, format_string))
